...
def transform_pos_angle(position, orientation, scale=1):
    ...
    if orientation >= 0.0 and orientation <= np.pi/2:
        theta = (np.pi/2) - orientation
    else:
        theta = 2*np.pi - orientation
        theta += np.pi/2
    angle = [0, 0, theta]
    return pos, angle
...
@timeit
def follow_path(robot, init_position, get_marker_object, vrep, clientID, debug=False, grid=None, **kw):
    try:
