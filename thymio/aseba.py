...
class Aseba(object):
    ...
    def __init__(self, system_bus=False, dummy=False):
        ...
        if not dummy:
            ...
            self.dispatch_handler = self.events.connect_to_signal(
                'Event', self._dispatch_events)
        ...
        for n in root.iter("node"):
            if 'name' in n.attrib:
                evtfilter = dbus.Interface(
                    self.bus.get_object(
                        'ch.epfl.mobots.Aseba', "/events_filters/%s" % n.attrib['name']),
                    dbus_interface='ch.epfl.mobots.EventFilter')
                evtfilter.Free()
    ...
    def _dispatch_events(self, *args):
        ...
        if key:
            ...
            if len(self._events_periods[key]) == Aseba.NB_EVTS_FREQ:
                self.events_freq[key] = 1. / \
                    (sum(self._events_periods[key]) /
                     float(Aseba.NB_EVTS_FREQ))
...
if __name__ == '__main__':
    ...
    with Aseba(options.system) as aseba:
        ...
        print("Get variable temperature: ",
              aseba.get("thymio-II", "temperature"))
